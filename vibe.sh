# WARNING: THIS FILE SHOULD NOT BE MODIFIED BY HAND BUT ONLY USING VIBE CODING, 
# WITH A RARE EXCEPTION FOR IMPROVEMENT OF PROMPTS AND SMALL FIXES.

# This function was entirely generated by AI through the Vibe Coding process. 
# The Vibe Coding process demonstrates how AI can augment or even replace developer tasks, streamlining the coding workflow and enhancing productivity.
vibe() {
  ...
  local VIBE_SYSTEM_PROMPT="You are a helpful assistant who answers questions clearly and directly, focusing only on the essential information.\nWhen asked to write or modify code in a file, always provide the entire content of the file, including unchanged lines and your modifications. Do not include any explanations, additional comments or markdown tags—only output the complete, updated file. Always ensure that the entire file contents are returned in your response, preserving all content unless explicitly instructed otherwise. Never add markdown code block quotes or any additional formatting—output the file content exactly as it should appear in the file."
  ...
  local target_type=""
  local ask_target_prompt="Determine if the target of the instructions is a file or the stdout. I need to know if a file should be created/modified or the result should be echoed in the terminal. Here is the instructions: \n\"\"\"$instructions\n\"\"\".\nIf it is a file, please provide the filename, but only the filename, nothing else. If it is stdout, please say 'stdout'."
  ...
  if [ ${#args[@]} -lt 2 ]; then
    ...
    target_type="$(chatgpt --role "$VIBE_SYSTEM_PROMPT" "$ask_target_prompt" | head -n 1 | tr -d '\"')"
